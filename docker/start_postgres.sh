...
if [ -n "${DB_USER_ICAT}" ]; then
  echo "Creating user \"${DB_USER_ICAT}\"..."
  echo "CREATE ROLE ${DB_USER_ICAT} WITH LOGIN PASSWORD 'test';" |
    sudo -u postgres -H postgres --single \
      -c config_file=${PG_CONFDIR}/postgresql.conf -D ${PG_CONFDIR}

fi
if [ -n "${DB_NAME_ICAT}" ]; then
  echo "Creating database \"${DB_NAME_ICAT}\"..."
  echo "CREATE DATABASE \"${DB_NAME_ICAT}\";" | \
    sudo -u postgres -H postgres --single \
     -c config_file=${PG_CONFDIR}/postgresql.conf -D ${PG_CONFDIR}

  if [ -n "${DB_USER_ICAT}" ]; then
    echo "Granting access to database \"${DB_NAME_ICAT}\" for user \"${DB_USER_ICAT}\"..."
    echo "GRANT ALL PRIVILEGES ON DATABASE \"${DB_NAME_ICAT}\" to ${DB_USER_ICAT};" |
      sudo -u postgres -H postgres --single \
      -c config_file=${PG_CONFDIR}/postgresql.conf -D ${PG_CONFDIR}
  fi
fi
